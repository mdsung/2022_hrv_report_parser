...
class Parser:
    ...
    def process(self):
        ...
        return General.from_html(html_chucks[0]),\
    ...
    def create_html_generator(self):
        ...
        keyword_indexes = [self.html_list.index(f'<h3>{key}</h3>') for key in keywords]
        keyword_indexes.append(len(self.html_list))
        
        return [self.html_list[keyword_indexes[i]:keyword_indexes[i+1]] for i in range(len(keyword_indexes) - 1)]
...
class General:
    ...
    def from_html(cls, html_list):
        ids = [i for i, html in enumerate(html_list) if 'class="rowtitle"' in html]
        args = [remove_td(html_list[rowtitle_index + 1]) for rowtitle_index in ids]
...
class TimeDomain:
    ...
    def from_html(cls, html_list):
        ids = [i for i, html in enumerate(html_list) if 'class="rowtitle"' in html]
        args = [parse_number(remove_td(html_list[rowtitle_index + 1])) for rowtitle_index in ids]
...
class FrequencyDomain:
    ...
    def from_html(cls, html_list):
        ...
        max_index = [i for i, html in enumerate(html_list) if '</table>' in html][1]
